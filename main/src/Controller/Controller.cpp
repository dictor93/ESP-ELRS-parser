...
void Controller::convertElrsToMotorsDirection(crsf_channels_t *channels) {
    ...
    printf("lSpeedAbsolute: %d, rSpeedAbsolute: %d\n", lSpeedAbsolute,
           rSpeedAbsolute);
    motorsState.ml_speed =
        (uint16_t)(sqrt(abs(lSpeedAbsolute)) * sqrt(maxChannelVal));
    motorsState.mr_speed =
        (uint16_t)(sqrt(abs(rSpeedAbsolute)) * sqrt(maxChannelVal));
    ...
    this->motors->setSpeed(motorsState.ml_speed, motorsState.mlDir,
                           motorsState.mr_speed, motorsState.mrDir);
}

Controller::Controller(PWMMotorsControl *motors) { this->motors = motors; };
void Controller::packageReceived(crsf_channels_t *channels) {
